...
#endif
{
    ...
    int retryCount = 0;
    while (inter->data_list.size > 0 && retryCount < 6) {
...
static int ta_batch_consumer_flush(void *this_) {
    ...
    if (ret != 0)
    {
        td_logInfo("create pthread error!");
    }
...
static int ta_batch_consumer_close(void *this_) {
    ...
    td_logInfo("TDAsyncBatchConsumer close.");
...
int td_init_consumer(struct TDConsumer **ta, const TDConfig *config) {
    ...
    *ta = consumer;
    ...
    td_logInfo("TDAsyncBatchConsumer init. ServerUrl: %s, appId: %s", inter->push_url, inter->appid);
